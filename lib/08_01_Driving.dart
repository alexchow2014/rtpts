...
class _drivingState extends State<driving> {
  ...
  void getLocation() {
    ...
    a1 = LatLng(globals.lat_01, globals.lng_01);
    b1 = LatLng(v001, v011);
  }
  ...
  Widget build(BuildContext context) {
    ...
    var Zoom;
    var time_tmp_01 = globals.result[globals.choice - 1]["driving_time"];
    var time_tmp1_01 = double.parse("$time_tmp_01")/60;
    var time_01 = time_tmp1_01.round();

    var vc01 = (globals.lat_01 + v001) / 2;
    var vc11 = (globals.lng_01 + v011) / 2;

    final LatLng _center = LatLng(vc01, vc11);

    if (globals.lat_01 - v001 >= 0) {
      if (globals.lat_01 - v001 <= 0.05) {Zoom = 13.0;}
      else if (globals.lat_01 - v001 <= 0.1) {Zoom = 12.0;}
      else if(globals.lat_01 - v001 <= 0.2){Zoom = 11.0;}
      else {Zoom = 10.0;}
    }
    else if (globals.lat_01 - v001 <= 0) {
      if (v001 -globals.lat_01 <= 0.05) {Zoom = 13.0;}
      else if (v001 -globals.lat_01 <= 0.1) {Zoom = 12.0;}
      else if (v001 -globals.lat_01 <= 0.2) {Zoom = 11.0;}
      else {Zoom = 10.0;}
    }

    if (globals.lng_01 - v011 >= 0){
      if (globals.lng_01 - v011 <= 0.05 && Zoom < 13.0) {Zoom = 13.0;}
      else if (globals.lng_01 - v011 <= 0.1 && Zoom < 12.0) {Zoom = 12.0;}
      else if (globals.lng_01 - v011 <= 0.2 && Zoom < 11.0) {Zoom = 11.0;}
      else if (globals.lng_01 - v011 > 0.2) {Zoom = 13.0;}
    }
    else if (globals.lng_01 - v011 <= 0){
      if (v011 - globals.lng_01 <= 0.05 && Zoom < 13.0) {Zoom = 13.0;}
      else if (v011 - globals.lng_01 <= 0.1 && Zoom < 12.0) {Zoom = 12.0;}
      else if (v011 - globals.lng_01 <= 0.2 && Zoom < 11.0) {Zoom = 11.0;}
      else if (v011 - globals.lng_01 > 0.2) {Zoom = 10.0;}
    }
    ...
    _launchURL() async {
      ...
      var url = 'https://www.google.com/maps/dir/?api=1&origin='+globals.lat_01.toString()+','+globals.lng_01.toString()+'&destination='+dn.toString()+','+de.toString()+'&travelmode=driving';
    ...
    return WillPopScope(
      ...
      child: MaterialApp(
        home: new Scaffold(
          appBar: AppBar(
            title: Text('Driving Route'), backgroundColor: Colors.green[700],
                  ...
                  children: <Widget>[
                    new InkWell(
                      onTap: () {
                        ...
                        Navigator.of(context).push(new MaterialPageRoute(builder: (BuildContext context) => new transit()));
                      },
                      child: new Container(
                        ...
                        width: width/2 - 40.0,
                        child: Icon(Icons.directions_transit, size: 42.0),
                      ),
                    ),
                    new InkWell(
                      onTap: () {
